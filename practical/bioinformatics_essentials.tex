...
\end{tcolorbox}
    ...
    This will switch the Linux user from \texttt{manager} to user
\texttt{other} to allow us to start from an empty non-configured set up
without conda installed. Then run the following commands:
...
\end{tcolorbox}
    ...
    \begin{figure}
\centering
\includegraphics{img/other_user.png}
...
(\texttt{other}) you should see a message indicating that conda is not
installed.

    \texttt{conda:\ command\ not\ found}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
mkdir\PY{+w}{ }\PYZhy{}p\PY{+w}{ }\PYZti{}/myminiconda
\end{Verbatim}
\end{tcolorbox}
...
\item
  ...
  \texttt{miniconda.sh} in the \texttt{\textasciitilde{}/myminiconda}
  directory:
\end{enumerate}
...
\PYZhy{}O\PY{+w}{ }\PYZti{}/myminiconda/miniconda.sh
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
bash\PY{+w}{ }\PYZti{}/myminiconda/miniconda.sh\PY{+w}{ }\PYZhy{}u\PY{+w}{ }\PYZhy{}b\PY{+w}{ }\PYZhy{}p\PY{+w}{ }\PYZti{}/myminiconda
\end{Verbatim}
\end{tcolorbox}
...
stored in \textasciitilde/myminiconda. Take a look:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }\PYZti{}/myminiconda
\end{Verbatim}
\end{tcolorbox}
...
\item
  ...
  \texttt{\textasciitilde{}/myminiconda/miniconda.sh} as it is no longer
  required:
\end{enumerate}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
rm\PY{+w}{ }\PYZhy{}rf\PY{+w}{ }\PYZti{}/myminiconda/miniconda.sh
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PYZti{}/myminiconda/bin/conda\PY{+w}{ }init\PY{+w}{ }bash
\end{Verbatim}
\end{tcolorbox}

    Restart the bash shell for the changes to take effect
...
\end{tcolorbox}
    ...
    If installed correctly, you should see (\textit{base}) at the start of
your prompt in your terminal.

    \begin{figure}
\centering
\includegraphics{img/conda_start.png}
\caption{Conda installed}
\end{figure}

    And run the command below as another check for conda is installed:
...
packages. When installing software packages via conda the packages and
their depenencies are pulled down/downloaded for these channels.
Examples of conda channels are \texttt{default} and \texttt{conda-forge}
and many bioinformatics software packages are available in the
\texttt{bioconda} channel.

By default, conda will search the \texttt{defaults} channel for a
software package.
...
\end{tcolorbox}
    ...
    If asked to proceed type \texttt{y} followed by \texttt{Enter}.

Alternatively you can configure conda to specify which chanells to
automatically search using the commands:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
conda\PY{+w}{ }config\PY{+w}{ }\PYZhy{}\PYZhy{}add\PY{+w}{ }channels\PY{+w}{ }defaults
...
\end{tcolorbox}
    ...
    These commands only need to be run once and all future conda installs
will search these channels. The order that the channels are searched
depends on the order that the channels are added. So when you add a
channel to your conda channel list it is automatically added to the top
of the list. So with the commands above the order to be searched will be
conda-forge, then bioconda and then defaults.
...
\end{tcolorbox}
    ...
    You should see the usage for samtools displayed.
...
\end{tcolorbox}
    ...
    The start of your terminal prompt will change from (samtools-1.17) back
to (base).

What happens if you try to run \texttt{samtools} now?
...
\end{tcolorbox}
    ...
    It is not always possible to remember all the software environments that
have been created. So to view the list of available environments use:
...
\end{tcolorbox}
    ...
    To list all the software packages installed inside a specific
environment:
...
\end{tcolorbox}
    ...
    This will list the software package \texttt{samtools} \textbf{and} also
all the other software packages that samtools depends on installed in
the \texttt{samtools-1.17} environment.
...
in the terminal window and enter the password when prompted:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
su\PY{+w}{ }\PYZhy{}l\PY{+w}{ }manager
\end{Verbatim}
\end{tcolorbox}
    ...
    \begin{figure}
\centering
\includegraphics{img/user_manager.png}
\caption{Switch to user manager}
\end{figure}
...
commands that were used to install it (\textbf{do not run these commands
here}).

    \texttt{wget\ https://github.com/sylabs/singularity/releases/download/v4.1.2/singularity-ce\_4.1.2-jammy\_amd64.deb}~\\
\texttt{sudo\ dpkg\ -i\ singularity-ce\_4.1.2-jammy\_amd64.deb}~\\
\texttt{rm\ singularity-ce\_4.1.2-jammy\_amd64.deb}
...
has a range of sub-commands (list, launcg, download etc.). The simplest
is \texttt{nf-core\ list}, which lists all available nf-core pipelines.
The output shows the latest version number, when that was released. If
the pipeline has been installed locally using Nextflow, it tells you
when that was and whether you have the latest version.
...
\tightlist
\item
  Use the nf-core help flag to print the usage for the nf-core list
  command
\item
  List all pipelines available from nf-core
\item
...
the execution of Nextflow workflows. It has many features including the
functionality to monitor the progress of your Nextflow pipeline runs via
a web interface.
...
are used by the pipelines.

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
mkdir\PY{+w}{ }/home/manager/my\PYZus{}singularity
\PY{n+nb}{export}\PY{+w}{ }\PY{n+nv}{NXF\PYZus{}SINGULARITY\PYZus{}CACHEDIR}\PY{o}{=}\PY{l+s+s2}{\PYZdq{}/home/manager/my\PYZus{}singularity/\PYZdq{}}
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }/home/manager/my\PYZus{}singularity/
\end{Verbatim}
\end{tcolorbox}
...
and a header row. An example file is shown below.
